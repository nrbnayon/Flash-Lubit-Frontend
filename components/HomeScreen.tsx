...
console.log("Voice id::", getAvatarsApi);

const API_BASE_URL = "http://192.168.10.251:8000";
...
const handleReplayDialogue = async () => {
  ...
  try {
      ...
      try {
        await new Promise((resolve, reject) => {
          ...
          audioElement.addEventListener("canplaythrough", () => {
            ...
            if (!replayActive) {
              resolve(); // Exit if stopped during loading
              return;
            }
    ...
    toast.error(`Failed to replay dialogue: ${error.message}`);
  } finally {
    setIsPlaying(false);
    replayActive = false;
    if (currentAudioRef.current) {
